...
### バックグラウンド動作
- **重要**: アプリがバックグラウンドに移行すると、周期送信タイマーがiOSによって一時停止されます
- これにより、コマンド送信が停止し、200ms後にデッドマンタイムアウトが発動します
- **結果**: ロボットは自動的に安全停止状態（FAULT）に遷移します
- **推奨**: 操作中はアプリを前面に保ち、iPadの自動ロックを無効化してください
