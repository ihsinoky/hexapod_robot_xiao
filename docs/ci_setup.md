...
現在、このリポジトリの GitHub Actions ワークフローには、コミットメッセージで CI をスキップするための
特別な仕組み（例: `[skip ci]` のような独自ルール）は設定されていません。GitHub Actions も
デフォルトではコミットメッセージ中の `[skip ci]` を解釈して CI を停止することはありません。

一時的に CI の実行を避けたい場合は、次のような運用で対応してください：

- WIP の変更は **Draft PR** として作成する
- CI を走らせたくないコミットは、PR を作成せずに個人ブランチ上で作業する
- 必要に応じてブランチ保護ルールで「いつ CI を必須にするか」を制御する
