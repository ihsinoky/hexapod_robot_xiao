...
### 8.2 iOS 実装者向け
- コマンド送信は **50–60 Hz** を推奨（最低 20 ms 間隔。過剰送信を避けつつ十分なデッドマン余裕を確保）
- デッドマンタイマーは **200 ms** でタイムアウトするため、操作がない場合でも PING を定期送信し、コマンドまたは PING の間隔が 150 ms を超えないようにする
- Telemetry の `last_cmd_age_ms` を監視し、**通常は 100 ms 未満**（最大でも 150 ms 未満）となるよう制御し、200 ms に近づく前に必ず送信
