...
**ERR_INVALID_CMD の詳細定義**

`ERR_INVALID_CMD` は、以下のいずれかに該当する「コマンドとして解釈できない／受理できない」メッセージを受信した場合に設定する。

- **未知 / 非対応の msg_type**  
  - 定義されていない `msg_type`、または本 FW バージョンがサポートしていない `msg_type` を受信した場合。
- **payload_len の不整合**  
  - 既知の `msg_type` に対して、仕様で定義された長さと異なる `payload_len` がヘッダに指定されている、または実際のペイロード長と一致しない場合。
- **パラメータ値の範囲外**  
  - コマンド自体の形式は正しいが、サーボ角度・チャンネル番号・フラグなどのフィールド値が仕様で定める許容範囲外であり、FW がそのコマンドを安全に実行できないと判断した場合。
- **不正な状態でのコマンド（状態不整合）**  
  - 現在の状態マシン上許可されていないコマンドを受信し、FW がそれを「無視」ではなく「拒否」として扱う実装とした場合（例: `state != ARMED` 時のサーボ制御コマンド）。  
  - 実装上、無視する設計とすることも可能だが、その場合は `ERR_INVALID_CMD` を必ずしも設定しなくてよい。
- **ヘッダ不正 / バージョン不一致**  
  - ヘッダの `version` が本仕様書で定義する値と異なる、またはその他のヘッダフィールドが明らかに不正（例: 予約ビットの異常値、あり得ない長さ）で、メッセージ全体を処理不能と判断した場合。

これらの条件のいずれかが発生した場合、FW は最新の `CMD_STATUS_UPDATE` において `error_code = ERR_INVALID_CMD` を設定し、上位（iOS アプリ）側がデバッグ・ログ収集に利用できるようにすることを推奨する。他のエラー（デッドマン、低電圧、I2C 障害など）については、それぞれの専用エラーコードを優先して使用する。
---
...
### 5.4 BLE切断時の挙動
BLE接続が切断された場合:
1. （接続が確立していた場合）現在の状態に関わらず、即座に FAULT 状態へ遷移し、サーボ出力を停止する
2. サーボ出力停止処理が完了した時点で、内部ステートマシンを DISARMED 状態にリセットする（FAULT 状態を切断後も保持しない）
3. 次回の BLE 再接続時には、常に DISARMED 状態から開始し、ARM コマンドを受信するまでは ARMED/FAULT には遷移しない
...
- デッドマンタイマーはおおよそ 10 ms 精度（タイムアウト 200 ms に対して ±10 ms 程度の誤差）でカウントすることを推奨
  （プラットフォームに余裕がある場合は 1 ms 精度など、より高精度で実装してもよい）
- サーボパルス幅の範囲チェック (500–2500 us) を実施し、**範囲外の値を受信した場合は以下の動作とする**:
  - 500 us 未満の値は 500 us に、2500 us を超える値は 2500 us にクランプして使用する
  - クランプが発生した事実は、少なくとも 1 フレーム以上テレメトリのエラー情報として報告すること（報告フォーマットは実装側でドキュメント化する）
  - このときコマンド自体はエラーとせず、ARMED/DISARMED/FAULT などの状態遷移には影響を与えない
- DISARMED/FAULT 状態では SET_SERVO コマンドを無視

### 8.2 iOS 実装者向け
- コマンド送信は **50–60 Hz** を推奨（16.67–20 ms 間隔。過剰送信を避けつつ十分なデッドマン余裕を確保）
- デッドマンタイマーは **200 ms** でタイムアウトするため、操作がない場合でも PING を定期送信する
- Telemetry の `last_cmd_age_ms` を監視し、**通常は 100 ms 未満**（最大でも 150 ms 未満）となるよう制御する
  - 50 Hz 送信の場合、BLE 遅延やパケットロスを考慮しても 5–7 パケット分の余裕がある
  - 150 ms を超えた場合は即座に PING を送信し、200 ms タイムアウトを回避する
