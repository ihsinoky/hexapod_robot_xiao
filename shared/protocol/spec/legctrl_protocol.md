...
### 5.4 BLE切断時の挙動
BLE接続が切断された場合:
1. 現在の状態に関わらず、即座に FAULT 状態へ遷移し、サーボ出力を停止する
2. 切断中は FAULT 状態を保持する（切断によるエラー状態を明示）
3. BLE 再接続時には、自動的に DISARMED 状態へ遷移する（正常状態に復帰）
4. ARM コマンドを受信するまでは ARMED 状態には遷移しない
